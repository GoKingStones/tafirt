...
public class Baidu extends Model<Baidu> {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final Baidu dao = new Baidu();
	
	public boolean insert(Baidu generalPage) {
...
//
//		boolean result = Db.save("baidu", record);
...
//		System.out.println(result);
	...
	public boolean update(Baidu generalPage, String sql) {
		// TODO Auto-generated method stub
		System.out.println("in baidu update");
...
//		boolean result = Db.update("baidu", record);
//		dao._setAttrs(generalPage);
...
//		System.out.println(flag);
	...
	public  Integer getMaxId(){
		Integer ret;
		String sql = "select max(id) from baidu";
		ret = Db.queryInt(sql);
		if(ret == null)
			ret = 0;
		return ret;
	...
	public boolean select(String url) {
		String sql = "select * from baidu where SourceURL = '" + url +"'";
		boolean flag = true;
		List<Baidu> pd = dao.find(sql);
		if(pd==null || pd.size()==0)
			flag = false;
		return flag;
	}
	
	public int[] insert(List<Baidu> pages) {
...
//		return result;
		...
		String sql = "insert into baidu(id, ZoneName, NewsTitle, NewsTime, SourceURL, body, task, num) values(?,?,?,?,?,?,?,?)";
	...
	public int deleteByTask(int taskID) {
		return Db.update("delete from baidu where task = ?",taskID);
	}
...
/*
	public boolean FindSig(String subsig) {
		...
		String sql = "select subsig from baidu";
...
*/
	 ...
	 * @return
	 */
	public List<Baidu> selectPage(String subsigStr, SimHasher sh) {	
//		System.out.println("in select page");
		List<Baidu> result = new ArrayList<Baidu>();
		for(String sub : subsigStr.split(",")){
			String sql = "select * from baidu where FIND_IN_SET('" + sub +"',subsig)";
//			System.out.println(sql);
		
			List<Baidu> page = dao.find(sql);
