...
/**
 * 柬埔寨西哈努克港经济特区 page processor
 * 
 */
public class ssezProcessor implements PageProcessor {
	...
	// ntwo控制的是新闻类型
	public static final String URL_LIST = "http://www\\.ssez\\.com/News\\.asp\\?(page=\\d+&)?None=3";
	public static final String URL_POST = "http://www\\.ssez\\.com/News\\.asp\\?nlt=\\d+&none=3&ntwo=\\d+";

	public ssezProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//			System.out.println(htmlStr);
			List<String> linkurls = html.xpath("//td[@colspan='2']//td[@align='center']/div[@class='digg4']/a").replace("(?is)<.*?>", "").all();
//			for(String l:linkurls)
//				System.out.println(l);
			if(linkurls.contains("&raquo;")){
				System.out.println("new page");
				String newurl = "";
				String tmp = page.getUrl().toString();	
//				System.out.println(tmp);
				if(!tmp.contains("page=")){
					newurl = "http://www.ssez.com/News.asp?page=2&None=3";
					System.out.println(newurl);
				}
				else{					
					String a = tmp.substring(tmp.indexOf("page=")+5,tmp.indexOf("&"));
					int num = Integer.valueOf(a);
					num++;
					String b = tmp.substring(tmp.indexOf("&"));
					newurl = tmp.substring(0,tmp.indexOf("page=")+5)+num+b;
					System.out.println(newurl);
				}
				page.addTargetRequest(newurl);
			}
			
	    	List<String> urls = html.xpath("//td[@colspan='2']//td/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//td[@colspan='2']//td[@align='right']").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//td[@colspan='2']//td/a").replace("(?is)<.*?>", "").all();
...
//							System.out.println("end setting");
						...
						generalPage.set("NewsTitle", names.get(i).replace("&middot;", ""));
