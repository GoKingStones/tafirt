...
/**
 * 中国印度尼西亚聚龙农业产业合作区 page processor
 * 
 */
public class jlaiczProcessor implements PageProcessor {
	...
	// ntwo控制的是新闻类型
	public static final String URL_LIST = "http://www\\.jlaicz\\.com/news\\.html";
	public static final String URL_POST = "http://www\\.jlaicz\\.com/news\\d+\\.html";

	public jlaiczProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//		System.out.println(page.getUrl());
		if (page.getUrl().regex(URL_LIST).match()) {
	    	...
	    	List<String> urls = html.xpath("//div[@class='new_content']//a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//div[@class='new_content']//a").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@class='new_content']//a").replace("(?is)<.*?>", "").replace("(?is)\\[.*\\]", "").all();
