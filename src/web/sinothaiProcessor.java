 ...
 * 
 */
public class sinothaiProcessor implements PageProcessor {
	...
	// ntwo控制的是新闻类型
	public static final String URL_LIST = "http://www\\.sinothaizone\\.com/news\\.php\\?cid=\\d+(&page=\\d+)?";
	public static final String URL_POST = "http://www\\.sinothaizone\\.com/newsinfo\\.php\\?id=\\d+";

	public sinothaiProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//			System.out.println(htmlStr);
			List<String> linkurls = html.xpath("//div[@class='proet_list']/span[@class='page_list']/a").replace("(?is)<.*?>", "").all();
...
//				System.out.println(tmp);
	    	...
	    	List<String> urls = html.xpath("//div[@class='proet_list']//li/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//div[@class='proet_list']//li/span").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@class='proet_list']//li/a").replace("(?is)<.*?>", "").all();
