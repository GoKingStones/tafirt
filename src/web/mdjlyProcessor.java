...
/**
 * 俄罗斯龙跃林业经贸合作区 page processor
 * 
 */
public class mdjlyProcessor implements PageProcessor {
	...
	public static final String URL_LIST = "http://www\\.mdjly\\.cn/index\\.php\\?p=news_list&lanmu=4";
	public static final String URL_POST = "http://www\\.mdjly\\.cn/index\\.php\\?p=news_show&id=\\d+&c_id=\\d+&lanmu=4";

	public mdjlyProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//		System.out.println(htmlStr);
		if (page.getUrl().regex(URL_LIST).match()) {
	    	...
	    	List<String> urls = html.xpath("//div[@id='news_list']//span/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//div[@id='news_list']//strong").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@id='news_list']//span/a").replace("(?is)<.*?>", "").all();
...
//							System.out.println("end setting");
						...
						generalPage.set("NewsTitle", names.get(i).replace("&middot;", "").replace(" ", ""));
