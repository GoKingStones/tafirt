 ...
 * 
 */
public class huaxinProcessor implements PageProcessor {
	...
	// http://www.huaxin-agri.com/cn/News.Asp?y=&Page=5
	public static final String URL_LIST = "http://www\\.huaxin-agri\\.com/cn/News\\.Asp(\\?y=&Page=\\d)?";
	public static final String URL_POST = "http://www\\.huaxin-agri\\.com/cn/News_view\\.Asp\\?id=\\d+";

	public huaxinProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//			System.out.println(htmlStr);
			List<String> linkurls = html.xpath("//div[@id='pagelist']/a").replace("(?is)<.*?>", "").all();
...
//				System.out.println(tmp);
	    	...
	    	List<String> urls = html.xpath("//article[@class='post']/h2[@class='posttitle']/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//article[@class='post']/div[@class='entry-utility']").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//article[@class='post']/h2[@class='posttitle']/a").replace("(?is)<.*?>", "").all();
