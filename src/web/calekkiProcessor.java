...
/**
 * 尼日利亚莱基自由贸易区－中尼经贸合作区 page processor
 * 
 */
public class calekkiProcessor implements PageProcessor {
	...
	public static final String URL_LIST = "http://www\\.calekki\\.com/xwzx/newsCategoryId=9(&FrontNews_list\\d{2}-\\d{13}_pageNo=\\d+&FrontNews_list\\d{2}-\\d{13}_pageSize=\\d+)?.html";
	public static final String URL_POST = "http://www\\.calekki\\.com/xwzx_xx/newsId=\\d+\\.html";

	public calekkiProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//		System.out.println(page.getUrl());
		if (page.getUrl().regex(URL_LIST).match()) {
			...
			List<String> linkurls = html.xpath("//div[@class='pageJump']/div[@class='number']/a").replace("(?is)<.*?>", "").all();
...
//				System.out.println(tmp);
				if(!tmp.contains("pageNo")){
					newurl = "http://www.calekki.com/xwzx/newsCategoryId=9&FrontNews_list01-1490879432616_pageNo=2&FrontNews_list01-1490879432616_pageSize=10.html";
//					System.out.println(newurl);
				}
				else{					
					String a = tmp.substring(tmp.indexOf("pageNo=")+7,tmp.lastIndexOf("&"));
//					System.out.println(a);
					int num = Integer.valueOf(a);
					num++;
					String b = tmp.substring(tmp.lastIndexOf("&"));
					newurl = tmp.substring(0,tmp.indexOf("pageNo=")+7)+num+b;
					System.out.println(newurl);
				}
				page.addTargetRequest(newurl);
			}
			
	    	List<String> urls = html.xpath("//div[@class='newstitle']//li[@class='title']//a").links().regex(URL_POST).all();
	    	List<String> times = html.xpath("//div[@class='newstitle']//li[@class='date']").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@class='newstitle']//li[@class='title']//a").replace("(?is)<.*?>", "").all();
