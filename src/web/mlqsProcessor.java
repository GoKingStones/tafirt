...
/**
 * 毛里求斯晋非经贸合作区 page processor
 * 
 */
public class mlqsProcessor implements PageProcessor {
	...
	public static final String URL_LIST = "http://www\\.mlqsydy\\.com/newslist(_\\d+)?/1923(/1942)?";
	public static final String URL_POST = "http://www\\.mlqsydy\\.com/newsdefault/1923/1942/\\d+";

	public mlqsProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//			System.out.println(htmlStr);
			List<String> linkurls = html.xpath("//div[@class='mainRight']//div[@class='page']/a").replace("(?is)<.*?>", "").all();
//			for(String l:linkurls)
//				System.out.println(l);
			if(linkurls.contains("下页")){
				System.out.println("new page");
				String newurl = "";
				String tmp = page.getUrl().toString();	
//				System.out.println(tmp);
				if(!tmp.contains("newslist_")){
					newurl = "http://www.mlqsydy.com/newslist_2/1923/1942";
					System.out.println(newurl);
				}
				else{					
					String a = tmp.substring(tmp.indexOf("newslist_")+9);
					a = a.substring(0,a.indexOf("/"));
					int num = Integer.valueOf(a);
					num++;
					newurl = tmp.substring(0,tmp.indexOf("newslist_")+9)+num+"/1923/1942";
					System.out.println(newurl);
				}
				page.addTargetRequest(newurl);
			}
			
	    	List<String> urls = html.xpath("//div[@class='mainRight']//div[@class='news']/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//div[@class='mainRight']//div[@class='news']//p//font").regex("\\d{4}\\.\\d{1,2}\\.\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@class='mainRight']//div[@class='news']//p//span").replace("(?is)<.*?>", "").all();
