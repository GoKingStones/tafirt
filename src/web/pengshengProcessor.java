...
/**
 * 乌兹别克斯坦鹏盛工业园 page processor
 * 
 */
public class pengshengProcessor implements PageProcessor {
	...
	public static final String URL_LIST = "http://www\\.pengshenguz\\.com/news/.+News/";
	public static final String URL_POST = "http://www\\.pengshenguz\\.com/html/\\d+\\.html";

	public pengshengProcessor(String domain, String charset, int taskId, String name, String source) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//			System.out.println(htmlStr);
			List<String> linkurls = html.xpath("//div[@class='t_page ColorLink']/a").replace("(?is)<.*?>", "").all();
//			for(String l:linkurls)
//				System.out.println(l);
			if(linkurls.contains("下一页")){
				System.out.println("having new page");
				try {
					throw new ServiceException("乌兹别克斯坦鹏盛工业园有了新的列表页，需更改代码结构");
...
//					newurl = "http://www.huaxin-agri.com/cn/News.Asp?y=&Page=2";
//					System.out.println(newurl);
//				}
//				else{					
//					String a = tmp.substring(tmp.indexOf("Page=")+5);
//					int num = Integer.valueOf(a);
//					num++;
//					String b = tmp.substring(0,tmp.indexOf("Page=")+5);
//					newurl = b+num;
//					System.out.println(newurl);
//				}
//				page.addTargetRequest(newurl);
	    	...
	    	List<String> urls = html.xpath("//div[@class='ArticleList']//td[@class='fw_t']/a").links().regex(URL_POST).all();
	    	// 要注意时间的格式！
	    	List<String> times = html.xpath("//div[@class='ArticleList']//td[@class='fw_s']").regex("\\d{4}-\\d{1,2}-\\d{1,2}").all();
	    	List<String> names = html.xpath("//div[@class='ArticleList']//td[@class='fw_t']/a").replace("(?is)<.*?>", "").all();
