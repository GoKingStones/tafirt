...
//	public static final String URL_POST = "http://www\\.setc-zone\\.com/system/\\d{4}/\\d{2}/\\d{2}/\\d+\\.shtml";
	...
	public testProcessor(String domain, String charset, int taskId, String name, String source, String official_url) {
		super();
		this.domain = domain;
		this.taskId = taskId;
		this.name = name;
...
//				System.out.println("===========================");
//			}
			List<Baidu> insertList = new ArrayList<>();
			List<Baidu> updateList = new ArrayList<>();
			
			for(int i=0;i<urls.size();i++){

//				System.out.println(urls.get(i));	
							
				Baidu Page = new Baidu();
				int num=0;
...
//						System.out.println(generalPage.get("subsig"));
						...
						List<Baidu> result = Page.dao.selectPage(subsigStr, sh);
						if(!result.isEmpty()){
							/*
							 * 存在则执行update语句，num + 1
							 */
							System.out.println("same page have already been crawed!");
							for(Baidu ExPage : result){
								num = ExPage.get("num");
//								System.out.println(num);
								num = num+1;
